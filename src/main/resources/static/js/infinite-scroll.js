...
(function ($, window, document, undefined) {
    ...
    Infinitus.prototype = {
        init: function () {
            ...
            if (self.options.workOnStart) action();

            self.scrollHandler = function () {
                if (!self.working && self.isAtBottom(self.options.bottomMargin)) {
                    action();
                }
            };
        ...
        isAtBottom: function (bottomMargin) {
            var contentOffset = this.element.offset(),
                elementBottom = this.element.height() + contentOffset.top,
                scrollBottom = this.window.scrollTop() + this.window.height() + bottomMargin;
